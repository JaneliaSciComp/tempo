classdef UFMFFile < handle
    ...
    properties
        ...
        frameRate = []          % The frame rate for fixed rate movies, empty for variable rate.
        frameCount = 0          % The number of frames in the movie.
        
        printStats = false      % If true then statistics will be printed while creating a file.
    end
    ...
    properties (Access=private)
        ...
        boxesAreFixedSize           % user-definable, if true then all ranges of foreground pixels must be the same size
        ...
        % Writing (user-definable)
        bgSampleCount = 5           % update the model with this many sample frames
    	bgSampleSeconds = 10        % update the model with a new sample frame every this many seconds
    	bgThreshold = 10            % pixels that differ more than this from the model are considered foreground
        minimizeBoxes               % whether to attempt to reduce the number of boxes per frame
        smallestBoxSize             % if minimizeBoxes is true then any foreground boxes smaller than this many pixels will not be included in a frame
        % (private)
        bgModel                     % a struct containing the background image, etc.
        frameIndex
        frameStats
    end
    ...
    methods
        ...
        function obj = UFMFFile(filePath, mode, varargin)
            ...
            elseif strcmp(mode, 'write')
                ...
                parser = inputParser;
                parser.addParamValue('BackgroundThreshold', 10, @(x) isnumeric(x) && x > 0);
                parser.addParamValue('BackgroundSampleCount', 10, @(x) isnumeric(x) && x > 0);
                parser.addParamValue('BackgroundSampleSeconds', 10, @(x) isnumeric(x) && x > 0);
                parser.addParamValue('FrameRate', [], @(x) isnumeric(x) && x > 0);
                ...
                obj.bgThreshold = parser.Results.BackgroundThreshold;
        ...
        function [frameNum, frameTime] = addFrame(obj, frameImage, frameTime)
            ...
            if isempty(obj.frameIndex.locLoc)
                ...
                if isempty(obj.bgModel.meanImage)
                    ...
                    obj.updateBGModel(frameImage, frameTime);
                    obj.writeKeyFrame(frameTime);
                else
                    % Write out the initial key frame generated by the user provided samples.
                    obj.writeKeyFrame(0.0);
                end
            end
            
            % Update the background model if called for.
            if obj.bgModel.isAutomatic && ...
               obj.bgModel.sampleCount < obj.bgSampleCount && ...
               obj.bgModel.lastSampleTime + obj.bgSampleSeconds <= frameTime
                obj.updateBGModel(frameImage, frameTime);
                obj.writeKeyFrame(frameTime);
            end
    ...
    methods (Access = private)
        ...
        function setupBGModel(obj)
            ...
            obj.frameIndex.frame = struct('loc', cast([], 'int64'), 'timestamp', []);
            obj.frameIndex.keyframe.mean = struct('loc', cast([], 'int64'), 'timestamp', []);
            
            obj.bgModel.meanImage = [];
            obj.bgModel.sampleCount = 0;
            obj.bgModel.lastSampleTime = -inf;
            obj.bgModel.isAutomatic = true;
        end
        
        
        function updateBGModel(obj, frameImage, timeStamp)
            % set time of this update
            obj.bgModel.lastSampleTime = timeStamp;
            
            % update the count of frames used for the model
            obj.bgModel.sampleCount = obj.bgModel.sampleCount + 1;
            ...
            if isempty(obj.bgModel.meanImage)
                obj.bgModel.meanImage = uint8(frameImage);
            else
        ...
        function [boundingBoxes, diffImage] = subtractBackground(obj, frameImage)
            ...
            if obj.boxesAreFixedSize
                ...
                if obj.minimizeBoxes && ~isempty(boundingBoxes)
